...
def gold_department_monthly_sales(logger,batch_id):
    try:
        gold_fact_sales = get_silver_table_reader('gold_fact_sales',engine= engine)

        if gold_fact_sales.empty:
            logger.warning(f"[GOLD][gold_department_monthly_sales][{batch_id}] No data found in gold_fact_sales")
            return
        ...
        logger.error(f"[GOLD BUILD FAILED: gold_department_monthly_sales][{batch_id}] {e}")
        raise